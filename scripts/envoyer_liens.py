...
def lire_liens_manuels() -> list[str]:
    ...
    contenu = FICHIER_LIENS.read_text(
        encoding="utf-8"
    )
...
def nom_fichier_log(
    ...
    return (
        f"{prefixe}-{premier_caractere}.txt"
    )
...
def écrire_journaux(
    ...
    for identifiant in sorted(identifiants):
        ...
        fichier = DOSSIER_LOG / nom_fichier_log(
            identifiant
        )

        if chemin not in groupes[fichier][base]:
            groupes[fichier][base].append(
                chemin
            )

    fichiers_utiles = set(groupes)

    for ancien_fichier in DOSSIER_LOG.glob(
        "*.txt"
    ):
    ...
    for tentative in range(
        ...
        try:
            ...
            print(
                ...
                f"{NOMBRE_TENTATIVES} échouée : "
                f"{erreur}"
            )

    if dernière_erreur is not None:
        raise dernière_erreur

    raise RuntimeError(
        "Échec inconnu du téléchargement."
    )


def extraire_magnets(
    html: str,
) -> list[str]:
...
def construire_url(
    ...
    """Remplace la plage ou l'astérisque par le numéro."""
    if numéro is None:
        return modèle
    ...
    for modèle in lire_sources():
        ...
        for numéro in pages:
            ...
            try:
                ...
                print(
                    f"  Arrêt du scan de cette source : "
                    f"{erreur}"
                )
                break
...
def supprimer_liens_envoyés(
    ...
    def remplacement(
        ...
        if identifiant in identifiants_envoyés:
            return ""

        return lien_original

    nouveau_texte = RE_LIEN.sub(
        remplacement,
        texte,
    )
...
def ajouter_liens_échoués(
    ...
    if (
        nouveau_contenu
        and not nouveau_contenu.endswith("\n")
    ):
        nouveau_contenu += "\n"

    nouveau_contenu += (
        "\n".join(liens_à_ajouter)
        + "\n"
    )
    ...
    print(
        f"{len(liens_à_ajouter)} lien(s) conservé(s) "
...
def main() -> None:
    ...
    supprimer_liens_envoyés(
        envoyés
    )

    ajouter_liens_échoués(
        échoués
    )
